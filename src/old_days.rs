...
pub fn day03(use_example: bool) -> io::Result<(usize, usize)> {
    ...
    let score1: usize = common.iter().map(|x| {
	match *x {
    ...
    let score2: usize = common.iter().map(|x| {
	match *x {
	    y if b'a' <= y && y <= b'z' => (y - b'a' + 1) as usize,
	    y if b'A' <= y && y <= b'Z' => (y - b'A' + 27) as usize,
	    _ => panic!()
...
#[allow(unused)]
fn day04(use_example: bool) -> io::Result<(usize, usize)> {
    let day = 4;
    let path = if use_example {
	format!("input/example{:02}.in", day)
    } else {
	...
	let a: Vec<_> = pieces[0].split("-").map(|x| x.parse::<usize>().unwrap()).collect();
	let b: Vec<_> = pieces[1].split("-").map(|x| x.parse::<usize>().unwrap()).collect();
...
fn day05(use_example: bool, num_stacks: usize, stack_height: usize) -> io::Result<(String, String)> {
    let day = 5;
    let path = if use_example {
	format!("input/example{:02}.in", day)
    } else {
    ...
    let r: Vec<u8> = (0..num_stacks).map(|x| stacks[x][stacks[x].len() - 1]).collect();
